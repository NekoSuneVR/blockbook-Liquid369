...
func (w *Worker) GetTransactionFromBchainTx(bchainTx *bchain.Tx, height int, spendingTxs bool, specificJSON bool) (*Tx, error) {
    ...
    r := &Tx{
        ...
        ShieldIns:        uint32(len(tempTx.VShieldIn)),
        ShieldOuts:       uint32(len(tempTx.VShieldOut)),
					...
					for i := range bchainTx.Vout {
						...
						if err == nil && bytes.Equal(addrDesc, vad) {
							...
							if !e {
								...
								if stakeContract := dogec.IsP2CSScript(addrDesc) {
									stakeContract = true
								}
								if stakeContract := dogec.IsP2CSSCriptOld(addrDesc) {
									stakeContract = true
								}
